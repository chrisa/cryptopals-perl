...
sub trigram_score {
    ...
    unless (defined $trigram_regexps) {
        _init_trigrams();
    }

    my $score = sum map { $trigram_regexps->{$_} * (scalar $text =~ $_) }
                    keys %$trigram_regexps;
