...
my $encrypt = sub { $aes->encrypt($_[0]) };
my $decrypt = sub { $aes->decrypt($_[0]) };

my $plaintext = '1234' x 100;
my $ciphertext = cbc_encrypt($encrypt, $iv, $plaintext);
$plaintext = cbc_decrypt($decrypt, $iv, $ciphertext);
print "$plaintext\n";

my $base64 = join '', map { chomp && $_ } <DATA>;
my $ciphertext2 = decode_base64($base64);
my $plaintext2 = cbc_decrypt($decrypt, $iv, $ciphertext2);
print "$plaintext2\n";
