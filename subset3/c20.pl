...
#
# (using 'max' here decrypts more, but gets things more wrong the less
# plaintext there is)
#
    ...
    for my $byte (0..255) {
        ...
        for my $ciphertext (@ciphertexts) {
            if (length($ciphertext) >= $i) {
                $bytes .= chr($byte) ^ substr $ciphertext, $i, 1;
            }
        }
        my $freqs = character_freqs($bytes);
        my $score = freq_score($freqs);
        push @scores, [$byte, $score];
    }

    my ($top) = sort { $a->[1] <=> $b->[1] } @scores;
    $keystream .= chr($top->[0]);
}

for my $ciphertext (@ciphertexts) {
    my $length = min (length $keystream, length $ciphertext);
    print xor_buffers(substr($ciphertext, 0, $length), substr($keystream, 0, $length));
    print "\n";
}
