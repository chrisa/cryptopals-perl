...
my $ciphertext = encrypted_data();

# pic.twitter.com/mmFhM1z9ty -- The CTR Penguin

my $half = (length $ciphertext) / 2;
my $mask = ("\0" x $half);
my $c1 = api_edit($ciphertext, 0, $mask);
my $c2 = api_edit($ciphertext, $half, $mask);

my $plaintext = xor_buffers($c1, $c2);
...
sub api_edit {
    my ($ciphertext, $offset, $newtext) = @_;
    $key ||= random_key();
    edit($ciphertext, $key, $offset, $newtext);
}

sub edit {
    my ($ciphertext, $key, $offset, $newtext) = @_;
    $nonce ||= "\0" x 8;
...
sub encrypted_data {
    ...
    $nonce ||= "\0" x 8;
    my $aes = Crypt::OpenSSL::AES->new($key);
    my $plaintext = do { local undef $/; <DATA> };
