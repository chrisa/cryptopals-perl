...
for my $key_length (2..20) {
    ...
    my $admin_mac = keyed_mac(\&my_sha1, $key, ($message . $glue . $admin));
...
sub forge_mac {
    ...
    my @regs = unpack("N5", $mac);
    my $forged_mac = my_sha1($admin, $fake_len, @regs);
...
sub md_padding {
    ...
    $padding .= "\0" x (55 - $len);
    $padding .= pack("N2", ($bit_len >> 32), ($bit_len & 0xffffffff));
